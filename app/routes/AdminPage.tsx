...
export default function AdminPage() {
  const [baseUrl] = useState("http://localhost:5177")
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        <h1 className="text-3xl font-extrabold text-center text-gray-900 mb-8">Panel de Administración</h1>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {adminLinks.map((link, index) => (
            <AdminButton key={index} title={link.title} href={`${baseUrl}${link.path}`} />
          ))}
        </div>
      </div>
    </div>
